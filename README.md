...
- [YYLO Benchmark](https://github.com/yylo-dev/yylo-benchmark) - Flexible, isolated evaluation layer for task prompts and Workflow Runner YAML that normalizes every case into one attempt contract, runs each candidate in a private fresh-repository workspace, and evaluates retained evidence with ordered deterministic and LLM judge profiles over immutable, receipt-linked records.
