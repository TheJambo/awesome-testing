...
- [Corund](https://github.com/Grade-Inc/corund-action) - Checks whether a pull request's new tests actually test the change: it reverts the PR's non-test code, re-runs the tests the PR added, and requires them to fail. Deterministic, with no model judgment, so a test that still passes against the old code is named as attached to nothing. Posts a receipt on the PR and is observe-only by default.
